...
func ListPurchases(ctx context.Context, logger *zap.Logger, db *sql.DB, userID string, limit int, cursor string) (*api.PurchaseList, error) {
	...
	FROM
			purchase
	%s
	ORDER BY purchase_time %s LIMIT $%s`, predicateConf, sortConf, strconv.Itoa(len(params)))
	...
	return &api.PurchaseList{ValidatedPurchases: purchases, NextCursor: nextCursorStr, PrevCursor: prevCursorStr}, nil
}
